...
import { getFileExtension, serializeData } from "@/lib/utils";
...
export const createUser = async (formData) => {
  try {
    ...
    return {
      ...
      data: serializeData(userResponse),
    };
  } catch (error) {
...
export const getEventsList = async () => {
  try {
    ...
    // Get active events for public display (no authentication required)
    const events = await Event.find({ 
      status: "ACTIVE",
      start_datetime: { $gte: new Date() } // Only upcoming events
    })
    .populate('inquiry_id', 'title client_name company_name') // Populate inquiry details if needed
    .populate('created_by', 'first_name last_name') // Populate creator details
    .sort({ start_datetime: 1 }); // Sort by start date ascending
    ...
    await errorLogger("getEventsList()", error.message);
    return {
      status: "ERROR",
      message: error.message,
      data: null,
...
export const unsubscribeClient = async (clientId) => {
  try {
    await connectDB();
    ...
    // Note: This assumes clients are stored in a separate collection
    // If clients are part of another model (User, Lead, etc.), adjust accordingly
    await connectDB();
    
    const result = await mongoose.connection.db.collection('clients').updateOne(
      { _id: clientId },
      { 
        $set: { 
          subscribed: false,
    ...
    return {
      ...
      data: { clientId: clientId, unsubscribed: true },
    };
  } catch (error) {
    await errorLogger("unsubscribeClient()", error.message);
    return {
      status: "ERROR",
      message: error.message,
      data: null,
...
export const getTargets = async () => {
  try {
    ...
    // Placeholder for targets collection - adjust based on your data structure
    const targets = await mongoose.connection.db.collection('targets').find({}).toArray();
    ...
    await errorLogger("getTargets()", error.message);
    return {
      status: "ERROR",
      message: error.message,
      data: null,
...
export const getCampaignReport = async (campaignId) => {
  try {
    ...
    // Get campaign report from campaigns collection
    const report = await mongoose.connection.db.collection('campaigns').findOne({ _id: campaignId });
    ...
    return {
      status: "OK",
      message: null,
      data: serializeData(report),
    };
  } catch (error) {
    await errorLogger("getCampaignReport()", error.message);
    return {
      status: "ERROR",
      message: error.message,
      data: null,
...
export const getCampaignReports = async () => {
  try {
    ...
    const reports = await mongoose.connection.db.collection('campaigns').find({}).toArray();
    ...
    await errorLogger("getCampaignReports()", error.message);
    return {
      status: "ERROR",
      message: error.message,
      data: null,
...
export const getFilteredInquiries = async (filters = {}) => {
  try {
    await connectDB();
    ...
    await errorLogger("getFilteredInquiries()", error.message);
    return {
      status: "ERROR",
      message: error.message,
      data: null,
...
export const getSoftwaresDetails = async () => {
  try {
    ...
    return {
      ...
      data: serializeData(transformedSoftwares),
    };
  } catch (error) {
...
export const getProposal = async (proposalId) => {
  try {
    ...
    await errorLogger("getProposal()", error.message);
    return {
      status: "ERROR",
      message: error.message,
      data: null,
...
export const getClient = async (clientId) => {
  try {
    ...
    // Query the clients collection directly
    const client = await mongoose.connection.db.collection('clients').findOne({ _id: clientId });
    ...
    await errorLogger("getClient()", error.message);
    return {
      status: "ERROR",
      message: error.message,
      data: null,
...
export const addUserCampaign = async (campaignData) => {
  try {
    await connectDB();
    ...
    // Create campaign document in campaigns collection
    const campaignId = uid.rnd();
    const campaign = {
      _id: campaignId,
      ...campaignData,
      user_id: user._id,
    ...
    await mongoose.connection.db.collection('campaigns').insertOne(campaign);
    ...
    await errorLogger("addUserCampaign()", error.message);
    return {
      status: "ERROR",
      message: error.message,
      data: null,
...
export const addClient = async (clientData, source = "marketing") => {
  try {
    await connectDB();
    ...
    await mongoose.connection.db.collection('clients').insertOne(client);
    ...
    await errorLogger("addClient()", error.message);
    return {
      status: "ERROR",
      message: error.message,
      data: null,
...
export const addTemplate = async (templateData) => {
  try {
    ...
    await mongoose.connection.db.collection('templates').insertOne(template);
    ...
    await errorLogger("addTemplate()", error.message);
    return {
      status: "ERROR",
      message: error.message,
      data: null,
