...
import { editTemplate } from "@/lib/actions";
import { logger, isObjectEmpty, cleanHtml } from "@/lib/utils";
import { names, emails, emptyHTML } from "@/lib/data";
...
const EditTemplateModal = ({ onClose, editingTemplate }) => {
  ...
  useEffect(() => {
    if (!isObjectEmpty(editingTemplate)) {
      handleEditingTemplate(editingTemplate);
      return;
    }
  }, [editingTemplate]);
  ...
  const handleEditingTemplate = (template) => {
  ...
  const handleTemplateSubmit = async () => {
    try {
      ...
      setWaiting(true);
      const response = await editTemplate(template);

      if (response.status === "ERROR") {
        setWaiting(false);
        logger("editTemplate()", response.message);
        toast.error("Unable to edit template.");
        return;
      }
      ...
      toast.success("Template has been edited.");
      ...
      setWaiting(false);
      logger("editTemplate()", error);
      toast.error("Something went wrong.");
    }
  };

  return (
    <div className="modal fade" id="edit-template-modal" tabIndex="-1" aria-hidden="true">
      <div className="modal-dialog modal-lg modal-dialog-centered" role="document">
        <div className="modal-content">
          <div className="modal-header">
            <h5 className="modal-title">Edit template</h5>
            <button type="button" className="btn-close" onClick={closeEditTemplateModal}></button>
          </div>
                ...
                <div className="col">
                  <div className="row">
                    <div className="col-md-4">
                      ...
                      <input type="text" id="key" autoComplete="off" className="form-control mb-3" value={template.key} onChange={onInputChange} readOnly disabled />
                    </div>
                  ...
                  <div className="row">
                    <div className="col-md-12">
                      ...
                      <HTMLEditor onClose={closeHTMLEditor} onChange={onHtmlChange} id={"#edit-template-modal"} defaultHTML={editingTemplate.content} />
                    </div>
                  </div>
                </div>
              </div>
