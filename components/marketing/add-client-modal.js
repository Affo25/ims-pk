...
const AddClientModal = ({ onClose, lists, companies }) => {
  ...
  const onTypeheadInputChange = (type, selected) => {
    ...
    if (type === "list") {
      ...
      setClient({ ...client, list: selected.map((item) => item.name).join(',') });
      return;
    }
  };
  ...
  const handleClientSubmit = async () => {
    try {
      ...
      const response = await addClient(client, "marketing");

      if (response.status === "ERROR") {
        setWaiting(false);
        logger("addClient()", response.message);
      ...
      setWaiting(false);
      logger("addClient()", error);
      toast.error("Something went wrong.");
    }
  };

  return (
    <div className="modal fade" id="add-client-modal" tabIndex="-1" aria-hidden="true">
      <div className="modal-dialog modal-lg modal-dialog-centered" role="document">
        <div className="modal-content">
          <div className="modal-header">
            <h5 className="modal-title">Add client</h5>
            <button type="button" className="btn-close" onClick={closeAddClientModal}></button>
          </div>
