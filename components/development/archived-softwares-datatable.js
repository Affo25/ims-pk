...
import ActivityDetailsModal from "@/components/development/activity-details-modal";
import SolutionDetailsModal from "@/components/development/solution-details-modal";
...
const ArchivedSoftwaresDataTable = ({ data }) => {
  ...
  const columns = [
    ...
    {
      header: "Solution / software details",
      cell: (props) => {
        const row = props.row.original;
  ...
  const openSolutionDetailsModal = async (solution) => {
    ...
    setSolutionDetails({
      portalEvents: solution.portal_events,
      scopeOfWork: solution.scope_of_work,
      softwareDetails: solution.software_details,
...
export default ArchivedSoftwaresDataTable;
