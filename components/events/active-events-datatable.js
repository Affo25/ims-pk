...
import FinishEventModal from "@/components/events/finish-event-modal";
import CancelEventModal from "@/components/events/cancel-event-modal";
import EditLogisticsModal from "@/components/events/edit-logistics-modal";
import EditEventNameModal from "@/components/events/edit-event-name-modal";
...
const ActiveEventsDataTable = ({ data }) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (props) => {
        ...
        return (
          <>
            <h6 className="fw-semibold mb-1">{row.software_status}</h6>
          </>
        );
      },
    },
          ...
          <>
            <div className="btn-group mb-2">
              ...
              <ul className="dropdown-menu" aria-labelledby="dropdown-menu-button">
                <li>
                  <button className="dropdown-item" onClick={() => openEditEventNameModal(row)}>
                    Edit event name
                  </button>
                </li>
                <li>
                  <button className="dropdown-item" onClick={() => openEditLogisticsModal(row)}>
                ...
                <li>
                  <button className="dropdown-item" onClick={() => openFinishEventModal(row)}>
                    Mark as finished
                  </button>
                </li>
                <li>
                  <button className="dropdown-item" onClick={() => openCancelEventModal(row)}>
                    Mark as cancelled
                  </button>
                </li>
                <li>
  ...
  const openEditEventNameModal = async (event) => {
    setModalState({ ...modalState, editEventName: true });
    setSelectedEvent(event);

    setTimeout(() => {
      openModal("edit-event-name-modal");
    }, 50);
  };

  const closeEditEventNameModal = () => {
    closeModal("edit-event-name-modal");

    setTimeout(() => {
      setModalState({ ...modalState, editEventName: false });
      setSelectedEvent({});
    }, 50);
  };

  const openEditLogisticsModal = async (event) => {
    setModalState({ ...modalState, editLogistics: true });
    setEditingLogistics(event);

    setTimeout(() => {
      openModal("edit-logistics-modal");
    }, 50);
  };

  const closeEditLogisticsModal = () => {
    closeModal("edit-logistics-modal");

    setTimeout(() => {
      setModalState({ ...modalState, editLogistics: false });
      setEditingLogistics({});
    }, 50);
  };
  ...
  const openFinishEventModal = (event) => {
    setModalState({ ...modalState, finishEvent: true });
    setSelectedEvent(event);

    setTimeout(() => {
      openModal("finish-event-modal");
    }, 50);
  };

  const closeFinishEventModal = () => {
    closeModal("finish-event-modal");

    setTimeout(() => {
      setModalState({ ...modalState, finishEvent: false });
      setSelectedEvent({});
    }, 50);
  };

  const openCancelEventModal = (event) => {
    setModalState({ ...modalState, cancelEvent: true });
    setSelectedEvent(event);
  ...
  const closeCancelEventModal = () => {
    ...
    setTimeout(() => {
      setModalState({ ...modalState, cancelEvent: false });
      setSelectedEvent({});
    }, 50);
  };
  ...
  //     saveAs(dataBlob, "clients_data.xlsx");

  //     toast.success("XLSX file has been downloaded.");
  //   } catch (error) {
  //     console.error("Error exporting XLSX:", error);
  //     toast.error("XLSX file could not be downloaded.");
  //   }
  // };
  ...
  return (
    <>
      ...
      {modalState.editEventName && <EditEventNameModal onClose={closeEditEventNameModal} selectedEvent={selectedEvent} />}
      {modalState.editLogistics && <EditLogisticsModal onClose={closeEditLogisticsModal} editingLogistics={editingLogistics} />}
      ...
      {modalState.finishEvent && <FinishEventModal onClose={closeFinishEventModal} selectedEvent={selectedEvent} />}
      {modalState.cancelEvent && <CancelEventModal onClose={closeCancelEventModal} selectedEvent={selectedEvent} />}
    </>
  );
};

export default ActiveEventsDataTable;
