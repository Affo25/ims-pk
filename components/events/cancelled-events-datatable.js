...
import CancelReasonModal from "@/components/events/cancel-reason-modal";
...
import RestoreCancelEventModal from "@/components/events/restore-cancel-event-modal";
...
const CancelledEventsDataTable = ({ data }) => {
        ...
        return (
          <>
            {row.cancelled_reason && (
              <button className="btn link-dark link-offset-2 text-decoration-underline p-0" type="button" onClick={() => openCancelReasonModal(row.cancelled_reason)}>
                View reason
              </button>
            )}
          </>
          ...
          <>
            <div className="btn-group mb-2">
              ...
              <ul className="dropdown-menu" aria-labelledby="dropdown-menu-button">
                <li>
                  <button className="dropdown-item" onClick={() => openRestoreCancelEventModal(row)}>
                    Mark as active
                  </button>
                </li>
                <li>
  ...
  const openCancelReasonModal = async (reason) => {
    setModalState({ ...modalState, cancelReason: true });
  ...
  const closeCancelReasonModal = () => {
    ...
    setTimeout(() => {
      setModalState({ ...modalState, cancelReason: false });
      setCancelReason("");
    }, 50);
  };

  const openRestoreCancelEventModal = (event) => {
    setModalState({ ...modalState, restoreCancelEvent: true });
    setSelectedEvent(event);

    setTimeout(() => {
      openModal("restore-cancel-event-modal");
    }, 50);
  };

  const closeRestoreCancelEventModal = () => {
    closeModal("restore-cancel-event-modal");

    setTimeout(() => {
      setModalState({ ...modalState, restoreCancelEvent: false });
      setSelectedEvent({});
    }, 50);
  };
  ...
  return (
    <>
      ...
      {modalState.cancelReason && <CancelReasonModal onClose={closeCancelReasonModal} cancelReason={cancelReason} />}
      {modalState.restoreCancelEvent && <RestoreCancelEventModal onClose={closeRestoreCancelEventModal} selectedEvent={selectedEvent} />}
    </>
  );
};

export default CancelledEventsDataTable;
