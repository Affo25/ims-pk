...
import UploadInvoiceModal from "@/components/accounts/upload-invoice-modal";
import ClearedPaymentModal from "@/components/accounts/cleared-payment-modal";
import ActivityDetailsModal from "@/components/accounts/activity-details-modal";
import EditAccountantDetailsModal from "@/components/accounts/edit-accountant-details-modal";
...
const UnpaidPaymentsDatatable = ({ data }) => {
          ...
          <>
            <div className="btn-group mb-2">
              ...
              <ul className="dropdown-menu" aria-labelledby="dropdownMenuButton">
                <li>
                  <button className="dropdown-item" onClick={() => openEditAccountantDetailsModal(row)}>
                ...
                <li>
                  <button className="dropdown-item" onClick={() => openClearedPaymentModal(row)}>
                    Mark as cleared
                  </button>
                </li>
                <li>
  ...
  const openEditAccountantDetailsModal = async (payment) => {
    setModalState({ ...modalState, editAccountantDetails: true });
    setSelectedPayment(payment);

    setTimeout(() => {
      openModal("edit-accountant-details-modal");
    }, 50);
  };

  const closeEditAccountantDetailsModal = () => {
    closeModal("edit-accountant-details-modal");

    setTimeout(() => {
      setModalState({ ...modalState, editAccountantDetails: false });
      setSelectedPayment({});
    }, 50);
  };

  const openClearedPaymentModal = async (payment) => {
    setSelectedPayment(payment);
    setModalState({ ...modalState, clearPayment: true });

    setTimeout(() => {
      openModal("cleared-payment-modal");
    }, 50);
  };

  const closeClearedPaymentModal = () => {
    closeModal("cleared-payment-modal");

    setTimeout(() => {
      setModalState({ ...modalState, clearPayment: false });
      setSelectedPayment({});
    }, 50);
  };
  ...
  const openUploadInvoiceModal = async () => {
    setModalState({ ...modalState, uploadInvoice: true });
    setTimeout(() => {
      openModal("upload-invoice-modal");
    }, 50);
  };

  const closeUploadInvoiceModal = () => {
    closeModal("upload-invoice-modal");

    setTimeout(() => {
      setModalState({ ...modalState, uploadInvoice: false });
      setSelectedPayment({});
      setInvoicesData([]);
    }, 50);
  };
  ...
  return (
    <>
      ...
      {modalState.editAccountantDetails && <EditAccountantDetailsModal onClose={closeEditAccountantDetailsModal} selectedPayment={selectedPayment} />}
      ...
      {modalState.invoices && <InvoicesModal onClose={closeInvoicesModal} invoicesData={invoicesData} selectedPayment={selectedPayment} onOpen={openUploadInvoiceModal} />}
      {modalState.uploadInvoice && <UploadInvoiceModal onClose={closeUploadInvoiceModal} selectedPayment={selectedPayment} />}
      {modalState.clearPayment && <ClearedPaymentModal onClose={closeClearedPaymentModal} selectedPayment={selectedPayment} />}
    </>
  );
};

export default UnpaidPaymentsDatatable;
