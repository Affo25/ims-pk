...
import ReversInquiryModal from "./revers-inquiry-modal";

const ConfirmedInquiriesDataTable = ({ data }) => {
          ...
          <>
            <div className="btn-group mb-2">
              ...
              <ul className="dropdown-menu" aria-labelledby="dropdownMenuButton">
                ...
                <li>
                  <button className="dropdown-item" onClick={() => openReversInquiryModal(row)}>
                    Revert to Submitted
                  </button>
                </li>
                <li>
  ...
  const openLostInquiryModal = (inquiry) => {
    ...
    setTimeout(() => {
      openModal("revert-inquiry-modal");
    }, 50);
  };

  const closeLostInquiryModal = () => {
    closeModal("revert-inquiry-modal");
  ...
  const openReversInquiryModal = async (inquiry) => {
    const response = await getProposals(inquiry.id);
    if (response.status === "ERROR") {
      toast.error(response.message);
      return;
    ...
    setProposalsData(response.data);
    setSelectedInquiry(inquiry);
    setModalState({ ...modalState, revertInquiry: true });
  ...
  const closeReversInquiryModal = () => {
    closeModal("revert-inquiry-modal");

    setTimeout(() => {
      setModalState({ ...modalState, revertInquiry: false });
      setProposalsData([]);
      setSelectedInquiry({});
    }, 50);
  };
  ...
  return (
    <>
      <div className="row mb-3">
        ...
        <div className="col-md-5" style={{ fontWeight: "bold", display: "flex", alignItems: "center" }}>{filteredCount} :Confirmed Inquiries</div>
      ...
      {modalState.revertInquiry && <ReversInquiryModal onClose={closeReversInquiryModal} selectedInquiry={selectedInquiry} proposalsData={proposalsData} />}
...
export default ConfirmedInquiriesDataTable;
