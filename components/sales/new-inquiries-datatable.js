...
import AddInquiryModal from "@/components/sales/add-inquiry-modal";
...
import DeleteInquiryModal from "@/components/sales/delete-inquiry-modal";
...
import { fishs } from "@/lib/data";

const NewInquiriesDataTable = ({ data }) => {
  ...
  const filteredData = useMemo(() => {
    return data.filter((item) => {
      const salespersonMatch = selectedSalesperson ? item?.user?.first_name === selectedSalesperson : true;
      const fishMatch = selectedFish ? item?.fish === selectedFish : true;
      return salespersonMatch && fishMatch;
    });
  }, [data, selectedSalesperson, selectedFish]);
  ...
  const columns = [
                ...
                {scopeOfWork[0].solution?.length !== 0 && (
                  <>
                    ...
                    <button
                      ...
                      type="button"
                      onClick={() => openSolutionDetailsModal(row)}
                    >
                      View details
                    </button>
    ...
    },
    {
      ...
      cell: (props) => {
        ...
        return (
          <>
            <h6 className="fw-semibold mb-0">{row.fish}</h6>
          </>
        );
      },
    },
          ...
          <>
            <div className="btn-group mb-2">
              ...
              <ul className="dropdown-menu" aria-labelledby="dropdown-menu-button">
                ...
                <li>
                  <button className="dropdown-item" onClick={() => openDeleteInquiryModal(row)}>
                    Delete inquiry
                  </button>
                </li>
                <li>
  ...
  const openDeleteInquiryModal = (inquiry) => {
    setModalState({ ...modalState, deleteInquiry: true });
    setSelectedInquiry(inquiry);

    setTimeout(() => {
      openModal("delete-inquiry-modal");
    }, 50);
  };

  const closeDeleteInquiryModal = () => {
    closeModal("delete-inquiry-modal");

    setTimeout(() => {
      setModalState({ ...modalState, deleteInquiry: false });
      setSelectedInquiry({});
    }, 50);
  };
  ...
  return (
    <>
      <div className="row mb-3">
        ...
        <div className="col-md-2">
          <div className="mt-3 mt-sm-0 p-0 d-flex align-items-center">
            <select className="form-select" id="fish-filter" value={selectedFish} onChange={handleFishChange}>
              <option value="">All</option>
              {fishs.map((per) => (
        ...
        <div className="col-md-1" style={{ fontWeight: "bold", display: "flex", alignItems: "center" }}>{filteredCount} :Inquiries</div>
      ...
      {modalState.addInquiry && <AddInquiryModal onClose={closeAddInquiryModal} companies={companies} />}
      {modalState.editInquiry && <EditInquiryModal onClose={closeEditInquiryModal} editingInquiry={editingInquiry} companies={companies} />}
      {modalState.deleteInquiry && <DeleteInquiryModal onClose={closeDeleteInquiryModal} selectedInquiry={selectedInquiry} />}
...
export default NewInquiriesDataTable;