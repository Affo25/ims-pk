...
import { editInquiry } from "@/lib/actions";
import { toast } from "react-hot-toast";
import { useRouter } from "next/navigation";
import { useEffect, useState } from "react";
import { Typeahead } from "react-bootstrap-typeahead";
import { logger, isEmail, isObjectEmpty } from "@/lib/utils";
import { countries, sources, solutions, extras, fishs, boundData } from "@/lib/data";

const EditInquiryModal = ({ onClose, editingInquiry, companies }) => {
  ...
  const onTypeheadInputChange = (type, selected, i) => {
    ...
    if (type === "fish") {
      ...
      if (selected.length === 0) {
        setInquiry({ ...inquiry, fish: "" });
        return;
      }

      if (selected[0]?.customOption) {
        setInquiry({ ...inquiry, fish: selected[0].label });
      } else {
        setInquiry({ ...inquiry, fish: selected[0] });
      }
    ...
    if (type === "bound") {
      ...
      if (selected.length === 0) {
        setInquiry({ ...inquiry, bound: "" });
        return;
      }

      if (selected[0]?.customOption) {
        setInquiry({ ...inquiry, bound: selected[0].label });
      } else {
        setInquiry({ ...inquiry, bound: selected[0] });
      }
  ...
  const handleEditingInquiry = (inquiry) => {
    ...
    setInquiry({
      ...
      bound: inquiry.bound ? inquiry.bound : "",
      ...
      fish: inquiry.fish ? inquiry.fish : "",
  ...
  const handleInquirySubmit = async () => {
    try {
      ...
      if (step === 2) {
        ...
        setWaiting(true);
        const response = await editInquiry(inquiry);

        if (response.status === "ERROR") {
          setWaiting(false);
          logger("editInquiry()", response.message);
