...
import LostReasonModal from "@/components/sales/lost-reason-modal";
...
const LostInquiriesDataTable = ({ data }) => {
    ...
    },
    {
      ...
      cell: (props) => {
        ...
        return (
          <>
            {row.lost_reason && (
              <button className="btn link-dark link-offset-2 text-decoration-underline p-0" type="button" onClick={() => openLostReasonModal(row.lost_reason)}>
                View reason
              </button>
            )}
          </>
  ...
  const openLostReasonModal = async (reason) => {
    setModalState({ ...modalState, lostReason: true });
  ...
  const closeLostReasonModal = () => {
    ...
    setTimeout(() => {
      setModalState({ ...modalState, lostReason: false });
      setLostReason("");
    }, 50);
  };
  ...
  return (
    <>
      <div className="row mb-3">
        ...
        <div className="col-md-5" style={{ fontWeight: "bold", display: "flex", alignItems: "center" }}>{filteredCount} :Lost Inquiries</div>
      ...
      {modalState.lostReason && <LostReasonModal onClose={closeLostReasonModal} lostReason={lostReason} />}
    </>
  );
};

export default LostInquiriesDataTable;
