...
const AddInquiryModal = ({ onClose, companies, addLead }) => {
  ...
  const handleInquirySubmit = async () => {
    try {
      if (step === 1) {
        ...
        if (inquiry.source === "") {
          toast.error("Please select the source.");
          return;
        }

        // if (inquiry.bound === "") {
        //   toast.error("Please select the bound.");
        //   return;
        // }
      ...
      if (step === 2) {
        ...
        const response = await addInquiry(inquiry);

        if (response.status === "ERROR") {
          setWaiting(false);
          logger("addInquiry()", response.message);
