...
const EditProposalModal = ({ onClose, selectedInquiry, selectedProposal }) => {
  ...
  useEffect(() => {
    if (isInitializing.current && !isObjectEmpty(selectedProposal)) {
      handleEditingProposal(selectedProposal);
      isInitializing.current = false;
      return;
    }

    calculateTotal();
  }, [selectedProposal, proposalFormValues]);

  const handleEditingProposal = (currentProposal) => {
    setProposalFormValues(
      currentProposal.details.map((detail) => ({
    ...
    setProposal({
      title: currentProposal.title + " [COPY]",
      subtotalAmount: currentProposal.subtotal_amount.toFixed(2),
      vatAmount: currentProposal.vat_amount.toFixed(2),
      totalAmount: currentProposal.total_amount.toFixed(2),
      details: currentProposal.details,
    });
  };
  ...
  return (
    <div className="modal fade" id="edit-proposal-modal" tabIndex="-1" aria-hidden="true">
      <div className="modal-dialog modal-lg modal-dialog-centered" role="document">
        <div className="modal-content">
          <div className="modal-header">
            <h5 className="modal-title">Edit proposal</h5>
